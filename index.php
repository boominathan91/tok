	...
	<div id="textchat">
		...
		<div id="user2" class="users" userId = "2">
			...
			if(!in_array(2,$user_ids)){
				echo '<button class="btn" onclick="invite(2)" id="btn_2">Invite</button>';					
			} else{				

				$query = "SELECT status,chat_id FROM chat_members WHERE chat_id IN ($chat_ids) AND user_id = '2'";
				$result = mysql_query($query);
				...
				if($rows['status'] == 0){
					...
					echo '<button class="btn" onclick="approve(2,'.$rows['chat_id'].')" id="btn_2">Approve</button>'; 				
				}else{
					echo '<button class="btn"  id="btn_2">Connected</button>'; 	
				}
			} 
			?>
